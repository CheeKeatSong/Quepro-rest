  ...
  db.task('verify-register-user', t => {
    ...
    .then(user => {
      // if (user.verificationcode == accountVerificationCode) {
        return t.one('INSERT INTO "User"(userid, firstname, lastname, email, password, mobilenumber, verificationCode, smsInterval, smsActivation, pushInterval, pushActivation, points) VALUES(DEFAULT, $1, $2, $3, $4, $5, 0, 60, true, 60, true, 0)', [user.firstname, user.lastname, user.email, user.password, user.mobilenumber]);
